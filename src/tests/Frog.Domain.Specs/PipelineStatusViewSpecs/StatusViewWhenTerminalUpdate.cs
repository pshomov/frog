...
namespace Frog.Domain.Specs.PipelineStatusViewSpecs
{
    ...
    public class StatusViewWhenTerminalUpdate : StatusViewAfterBuildStarterSpecBase
    {
        ...
        {
            Assert.That(BuildStatuses["http://fle"].Tasks[0].GetTerminalOutput().Content, Is.EqualTo("content1"));
        }

        [Test]
        public void should_return_empty_output_for_task_1()
        {
            Assert.That(BuildStatuses["http://fle"].Tasks[1].GetTerminalOutput().Content, Is.EqualTo(""));
        }
    }
