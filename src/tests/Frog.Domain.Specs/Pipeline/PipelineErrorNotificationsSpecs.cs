...
{
    [TestFixture]
    public class PipelineErrorNotificationsSpecs : BDD
    {
        ...
        {
            task1.Perform(Arg.Any<SourceDrop>()).Returns(new ExecTaskResult(ExecTask.ExecutionStatus.Error, -1));
        ...
        [Test]
        public void should_not_start_second_task_too()
        {
            task2.DidNotReceive().Perform(Arg.Any<SourceDrop>());
        }

        [Test]
        public void should_send_pipeline_ended_message_with_status_error()
        {
