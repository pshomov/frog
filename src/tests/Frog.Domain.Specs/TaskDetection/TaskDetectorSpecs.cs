...
namespace Frog.Domain.Specs.TaskDetection
{
    ...
    public class TaskDetectorSpecs : MsBuildSpecBase
    {
        ...
        public void should_have_msbuild_as_command()
        {
            ...
            Assert.That((items.First() as ShellTaskk).Command.Contains("msbuild.exe"), Is.True);
        }
        ...
        {
            Assert.That(items.Count(), Is.EqualTo(1));
            Assert.That((items.First() as ShellTaskk).Arguments.Contains("a1.sln"), Is.True);
        }
    }
    ...
    public class TaskDetectorUnixSpecs : MsBuildSpecBase
    {
        ...
        {
            Assert.That(items.Count(), Is.EqualTo(1));
            Assert.That((items.First() as ShellTaskk).Command.Contains("xbuild"), Is.True);
        }
    }
    ...
    public class TaskDetectorMultipleSolutionsAtTheRootWithBuildOneSpecs : MsBuildSpecBase
    {
        ...
        {
            Assert.That(items.Count(), Is.EqualTo(1));
            Assert.That((items.First() as ShellTaskk).Arguments, Is.EqualTo("Build.sln"));
        }
    }
    ...
    public class OneSolutionAnywhereGetsSelectedForBuildSpecs : MsBuildSpecBase
    {
        ...
        {
            Assert.That(items.Count(), Is.EqualTo(1));
            Assert.That((items.First() as ShellTaskk).Arguments, Is.EqualTo(Os.DirChars("fle\\flo\\a.sln")));
        }
    }
